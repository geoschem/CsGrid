function [ xMatLon, xMatLat ] = calcLLXMat( lonEdgeIn, latEdgeIn, lonEdgeOut, latEdgeOut )
%CALCLLXMAT MATLAB implementation of REGRID_A2A_MOD from GEOS-Chem
%   Original routine by S-J Lin. Calculates 1-D regridding vectors for conversion between
%   two regular lat-lon grids.

nLonIn  = length(lonEdgeIn)  - 1;
nLatIn  = length(latEdgeIn)  - 1;
nLonOut = length(lonEdgeOut) - 1;
nLatOut = length(latEdgeOut) - 1;
