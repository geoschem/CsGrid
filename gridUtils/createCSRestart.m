...
%   output_res: cs divisions per side (eg. c24 restart would have
%                output_res = 24)
...
% Ensure file is closed whether or not we get a clean exit
cleanupFn = onCleanup(@(x)netcdf.close(ncid));
...
xRegrid = calcHrzRegridMatGeneric(sprintf('C%i',output_res),genGridSpec('gmao4x5','geos5'),[]);
...
for varid = 0:numvars-1
    ...
    if (regexp(varname, '^SPC_.*$'))
        ...
        data = applyHrzRegridMatGeneric(data,xRegrid);
